...
def shopper():
    ...
    while items_on_notepad <= 20:
        if items_to_add and pencil.acquire(blocking=False):    # add item(s) to shared items_on_notepad
            """
            pencil.acquire(blocking=False) will check weather pencil (try-lock) is free
            and returns True (if no thread use it) and False (if it is used by another thread)
            
            We should always do if-else statement and for this condition. 
            In other case 'RuntimeError: release unlocked lock' will be fired.
            """
